                ...
                if line.tagih_ok:
                    if line.lokal_ok:
                        komersial_line = self.env['komersial.line'].create({
                            ...
                            'name': line.name.id,
                            'qty': line.qty,
                            'harga_inv': line.harga_lokal,
                            'harga_jual': line.cust_price,
                            })
                        ...
                        komersial_line = self.env['komersial.line'].create({
                            ...
                            'name': line.name.id,
                            'qty': line.qty,
                            'harga_inv': line.name.standard_price,
                            'harga_jual': line.cust_price,
                            'uom': line.uom,
                            })
...
#        self.cust = self.workorders_id.parent_id.name
    ...
    def _compute_cust(self):
        for rec in self:
            rec.cust = rec.workorders_id.cust_id.parent_id.name

    def _compute_cust_price(self):
        for rec in self:
            customer = self.env['cust.price'].search([('product_id','=',rec.name.id)])
            for cs in customer:
                if rec.cust == cs.name.name:
                    rec.cust_price = cs.price

    cust = fields.Char(string="Customer" , compute="_compute_cust", readonly=True)
    cust_price = fields.Float(string="Cust Price", compute="_compute_cust_price")
