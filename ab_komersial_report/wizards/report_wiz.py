...
class ReportRekapReportView(models.AbstractModel):
    ...
    def _get_report_values(self, docids, data=None):
        ...
        for rk in rekap:
            ...
            # rekapitulasi = self.env['komersial.line'].search([('komersial_id.cust_id.name','=',rk.cust_id.name)])
            # jppi_khs = 0
            # for rp in rekapitulasi:
            # for kli in rk.komersial_line_ids:
            #     jppi_khs = sum(kli.harga_jual)
            docs.append({
                'customer': rk.cust_id.name,
                'komersial_line_ids':rk.komersial_line_ids,
                'asset':rk.name.unit_id.name,
            })
